...
import { deleteAdminOverride, deleteAdminPlayer, loadAdminData, loadAdminGotwData, loadAdminOverrideHistory, loadDivisionWinnerData, mergeGames, resetAdminPassword, saveAdminPicks, setDivisionWinnerSettings, setGameOfWeek, setRegistrationOpen } from './api.js'
...
const dateKey = (kickoff, timeZone) => Object.fromEntries(new Intl.DateTimeFormat('en', { timeZone, year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(new Date(kickoff)).map(({ type, value }) => [type, value]))

export const startsBeforeBerlinMidnight = (game) => {
  if (!Number.isFinite(Date.parse(game?.kickoff))) return false
  const berlin = dateKey(game.kickoff, 'Europe/Berlin')
  const nflGameDay = dateKey(game.kickoff, 'America/New_York')
  return berlin.year === nflGameDay.year && berlin.month === nflGameDay.month && berlin.day === nflGameDay.day
}
...
export default function AdminPanel({ poolKey, token, browserGamesByPool, onPicksUpdated, onPlayerDeleted, onGamesUpdated }) {
  ...
  const [beforeMidnightOnly, setBeforeMidnightOnly] = useState(false)
  ...
  const storedGotwGames = (gotwData?.games ?? []).filter((game) => game.poolKey === gotwPoolKey)
  const gotwGames = mergeGames(storedGotwGames, browserGamesByPool?.[gotwPoolKey] ?? [])
  const eligibleGotwGames = beforeMidnightOnly ? gotwGames.filter(startsBeforeBerlinMidnight) : gotwGames
  const assignedGotwGameId = gotwGames.find((game) => game.gotw)?.id ?? ''
  const sortedGotwGames = [...eligibleGotwGames].sort((a, b) => {
  ...
  const assignGotw = async (gameId = selectedGotwGameId) => {
    ...
    try {
      ...
      const updatedPoolGames = gotwGames.map((game) => ({ ...game, gotw: Boolean(gameId) && game.id === gameId }))
      const updatedById = new Map(updatedPoolGames.map((game) => [game.id, game]))
      const nextGames = (gotwData?.games ?? []).map((game) => game.poolKey === gotwPoolKey ? updatedById.get(game.id) ?? game : game)
      ...
      onGamesUpdated(gotwPoolKey, updatedPoolGames)
