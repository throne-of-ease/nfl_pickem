...
function ScoreCell({ game }) {
  ...
  if (final || live) return <div className={`overview-score ${live ? 'live-text' : ''}`} title={live && detail ? `${detail}${liveProbability ? `; ${liveProbability}` : ''}` : final ? 'Final score' : 'Live score'}><strong>{game.awayScore}-{game.homeScore}</strong><small>{live ? 'LIVE' : 'FINAL'}</small>{detail && live && <small className="live-detail">{detail}</small>}{liveProbability && <small className="live-detail">{liveProbability}</small>}</div>
