...
export default function App() {
    ...
    const load = async () => {
      ...
      try {
        if (initial) {
          ...
          const result = await refreshLivePool(poolKey, loadedGamesByPool[poolKey] ?? [], { currentWeek, signal: controller.signal })
          if (controller.signal.aborted) return
          setLoadedGamesByPool((all) => ({ ...all, [poolKey]: result.games }))
  ...
  }, [poolKey, useFixtures, session?.access_token, session?.user?.id, refreshVersion, hasLiveGames, currentWeek])
  ...
  return <>
    ...
    <main id="top">
      {useFixtures && showSetup && <form className="player-setup" aria-label="Set up 3 players" onSubmit={registerPlayers}><div><h2>Register 3 rehearsal players</h2><p>This replaces local rehearsal players and picks on this device.</p></div>{playerNames.map((name, index) => <label key={index}>Player {index + 1}<input aria-label={`Player ${index + 1} name`} value={name} onChange={(event) => setPlayerNames((all) => all.map((item, i) => i === index ? event.target.value : item))} autoComplete="off" /></label>)}<button type="submit">Create players</button></form>}
       <section className="hero"><div className="hero-actions"><label>Pool <select aria-label="Pool" value={poolKey} onChange={(event) => setPoolKey(event.target.value)}>{enabledPools.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></label>{!useFixtures && <button type="button" title={currentWeek ? 'Refresh the current week from ESPN' : 'Older weeks use their cached slate'} onClick={() => setRefreshVersion((version) => version + 1)} disabled={isRefreshing || Boolean(baseGames.length) && !currentWeek}>{isRefreshing ? 'Refreshing…' : currentWeek ? 'Refresh' : 'Historical cache'}</button>}</div></section>
      ...
      {tab === 'models' && <section><div className="section-title"><div><h2>Model picks</h2><p>ESPN FPI probability and normalized no-vig moneyline, equal weighted.</p></div><div className="table-options"><label>Order by<select aria-label="Order models by" value={modelSort.key} onChange={(event) => setModelSort({ key: event.target.value, direction: event.target.value === 'kickoff' ? 'ascending' : 'descending' })}><option value="kickoff">Game time</option><option value="game">Game</option><option value="fpi">FPI</option><option value="fpiRank">FPI rank</option><option value="moneyline">Moneyline</option><option value="moneylineRank">Moneyline rank</option><option value="avg">AVG</option><option value="avgRank">AVG rank</option><option value="homeProbability">Home probabilities</option><option value="disagreement">Disagreement</option></select></label><button type="button" aria-label="Model sort direction" onClick={() => setModelSort((current) => ({ ...current, direction: current.direction === 'ascending' ? 'descending' : 'ascending' }))}>{modelSort.direction === 'ascending' ? 'Ascending' : 'Descending'}</button></div></div><div className="table-scroll"><table><thead><tr><th>Game</th><th>FPI</th><th>Rank</th><th>Moneyline</th><th>Rank</th><th>AVG</th><th>Rank</th><th>Home probabilities FPI / ML / AVG</th><th>Disagreement</th></tr></thead><tbody>{sortedModelRows.map(({ game, fpi, moneyline: ml, avg: pick, disagreement }) => <tr key={game.id}><td><div className="model-game"><span className="model-matchup"><TeamLogo team={game.away} />{game.away}<span>@</span><TeamLogo team={game.home} />{game.home}</span><time dateTime={game.kickoff}>{formatCETWeekday(game.kickoff)} {formatCETTime(game.kickoff)}</time></div></td><td><ModelPick pick={fpi} /></td><td>{fpi?.confidence ?? '—'}</td><td><ModelPick pick={ml} /></td><td>{ml?.confidence ?? '—'}</td><td><ModelPick pick={pick} /></td><td>{pick?.confidence ?? '—'}</td><td>{[fpi, ml, pick].map((item) => item ? `${(item.probability * 100).toFixed(1)}%` : '—').join(' / ')}</td><td>{disagreement === null ? 'Missing inputs' : `${(disagreement * 100).toFixed(1)} pts`}</td></tr>)}</tbody></table></div></section>}
