...
import { authenticate, clearSession, isCurrentPool, loadChartData, loadPool, loadRegistrationStatus, refreshLivePool, restoreSession, savePicks, updatePassword } from './api.js'
...
function StandingsTable({ board, pool, provisional, onProvisional }) {
  return <section className="standings-card" aria-labelledby="standings-title">
    <div className="standings-heading">
      ...
      <label className="provisional-toggle"><input type="checkbox" checked={provisional} onChange={(event) => onProvisional(event.target.checked)} /> Include provisional live scores</label>
    </div>
    <div className="table-scroll"><table className="standings-table" data-testid="standings-table" aria-label="Current standings"><thead><tr><th>Rank</th><th>Player</th><th>Points</th><th>Up to</th></tr></thead><tbody>{board.map((player, index) => <tr key={player.id}><th scope="row">{index + 1}</th><td><strong>{player.name}</strong></td><td>{player.points}</td><td>{player.points + player.potential}</td></tr>)}</tbody></table></div>
  </section>
}
...
export default function App() {
  ...
  return <>
      <header><a className="brand" href="#top"><span>NFL</span> PICK/26</a><nav aria-label="Main navigation">{[['overview', 'Overview'], ['games', 'My picks'], ['charts', 'Charts'], ['models', 'Models'], ['rules', 'Rules'], ...(isAdmin ? [['admin', 'Admin']] : [])].map(([item, label]) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{label}</button>)}</nav>{useFixtures ? <><button className="setup-button" type="button" aria-label="Set up 3 players" onClick={() => setShowSetup((open) => !open)}>Players</button><label className="user-switch"><span>Playing as</span><select aria-label="Active user" value={userId} onChange={(event) => setUserId(event.target.value)}>{appUsers.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label></> : <div className="signed-in"><span className="viewer-name">{viewerName || session.user.user_metadata?.display_name || session.user.user_metadata?.username || 'Player'}</span><button className="account-toggle" type="button" aria-label="Player options" aria-expanded={showAccountMenu} onClick={() => setShowAccountMenu((open) => !open)}>•••</button><div className={`account-actions ${showAccountMenu ? 'open' : ''}`}>{isAdmin && <button className="backup-button" type="button" data-testid="download-backup" onClick={exportBackup}>Backup</button>}<button type="button" aria-expanded={showPasswordForm} onClick={() => { setShowPasswordForm((open) => !open); setPasswordMessage(''); setShowAccountMenu(false) }}>Change password</button><button type="button" onClick={() => { clearSession(); initialLoadKey.current = ''; setSession(null); setAppUsers([]); setPicks({}); setIsAdmin(false); setShowAccountMenu(false) }}>Sign out</button></div></div>}</header>
    <main id="top">
      {useFixtures && showSetup && <form className="player-setup" aria-label="Set up 3 players" onSubmit={registerPlayers}><div><h2>Register 3 rehearsal players</h2><p>This replaces local rehearsal players and picks on this device.</p></div>{playerNames.map((name, index) => <label key={index}>Player {index + 1}<input aria-label={`Player ${index + 1} name`} value={name} onChange={(event) => setPlayerNames((all) => all.map((item, i) => i === index ? event.target.value : item))} autoComplete="off" /></label>)}<button type="submit">Create players</button></form>}
       <section className="hero"><div className="hero-actions"><label>Pool <select aria-label="Pool" value={poolKey} onChange={(event) => setPoolKey(event.target.value)}>{enabledPools.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></label>{!useFixtures && <button type="button" title={currentWeek ? 'Refresh the current week from ESPN' : 'Refresh this week from ESPN, including model data'} onClick={requestRefresh} disabled={isRefreshing}>{isRefreshing ? 'Refreshing…' : 'Refresh'}</button>}</div></section>
      ...
      {tab === 'overview' && <Overview players={appUsers} games={games} picksByUser={Object.fromEntries(appUsers.map((user) => [user.id, picksByUser[user.id]?.[poolKey] ?? []]))} history={chartHistory} pool={pool} provisional={provisional} onProvisional={setProvisional} />}
      ...
      {tab === 'charts' && <section><div className="section-title"><div><h2>Season charts</h2><p>Preseason, regular season, and postseason results.</p></div></div><StandingsTable board={board} pool={pool} provisional={provisional} onProvisional={setProvisional} /><div className="charts"><CumulativePointsChart history={chartHistory} />{games.length > 0 && <CurrentWeekChart current={current} />}<WeeklyPointsChart history={chartHistory} /><GotwChart history={chartHistory} /></div></section>}
      {tab === 'models' && <section><div className="section-title"><div><h2>Model picks</h2><p>ESPN FPI probability and normalized no-vig moneyline, equal weighted.</p></div></div><div className="table-scroll"><table><thead><tr><th aria-sort={modelSortOrder('kickoff')}><button className="table-sort-button" type="button" data-testid="models-sort-game" onClick={() => sortModelsBy('kickoff')}>Game{modelSortIndicator('kickoff')}</button></th><th aria-sort={modelSortOrder('fpi')}><button className="table-sort-button" type="button" data-testid="models-sort-fpi" onClick={() => sortModelsBy('fpi')}>FPI{modelSortIndicator('fpi')}</button></th><th aria-sort={modelSortOrder('fpiRank')}><button className="table-sort-button" type="button" data-testid="models-sort-fpi-rank" onClick={() => sortModelsBy('fpiRank')}>Rank{modelSortIndicator('fpiRank')}</button></th><th aria-sort={modelSortOrder('moneyline')}><button className="table-sort-button" type="button" data-testid="models-sort-moneyline" onClick={() => sortModelsBy('moneyline')}>Moneyline{modelSortIndicator('moneyline')}</button></th><th aria-sort={modelSortOrder('moneylineRank')}><button className="table-sort-button" type="button" data-testid="models-sort-moneyline-rank" onClick={() => sortModelsBy('moneylineRank')}>Rank{modelSortIndicator('moneylineRank')}</button></th><th aria-sort={modelSortOrder('avg')}><button className="table-sort-button" type="button" data-testid="models-sort-avg" onClick={() => sortModelsBy('avg')}>AVG{modelSortIndicator('avg')}</button></th><th aria-sort={modelSortOrder('avgRank')}><button className="table-sort-button" type="button" data-testid="models-sort-avg-rank" onClick={() => sortModelsBy('avgRank')}>Rank{modelSortIndicator('avgRank')}</button></th><th aria-sort={modelSortOrder('homeProbability')}><button className="table-sort-button" type="button" data-testid="models-sort-probability" onClick={() => sortModelsBy('homeProbability')}>Home probabilities FPI / ML / AVG{modelSortIndicator('homeProbability')}</button></th><th aria-sort={modelSortOrder('disagreement')}><button className="table-sort-button" type="button" data-testid="models-sort-disagreement" onClick={() => sortModelsBy('disagreement')}>Disagreement{modelSortIndicator('disagreement')}</button></th></tr></thead><tbody>{sortedModelRows.map(({ game, fpi, moneyline: ml, avg: pick, disagreement }) => <tr key={game.id}><td><div className="model-game"><span className="model-matchup"><TeamLogo team={game.away} />{game.away}<span>@</span><TeamLogo team={game.home} />{game.home}</span><time dateTime={game.kickoff}>{formatCETWeekday(game.kickoff)} {formatCETTime(game.kickoff)}</time></div></td><td><ModelPick pick={fpi} /></td><td>{fpi?.confidence ?? '—'}</td><td><ModelPick pick={ml} /></td><td>{ml?.confidence ?? '—'}</td><td><ModelPick pick={pick} /></td><td>{pick?.confidence ?? '—'}</td><td>{[fpi, ml, pick].map((item) => item ? `${(item.probability * 100).toFixed(1)}%` : '—').join(' / ')}</td><td>{disagreement === null ? 'Missing inputs' : `${(disagreement * 100).toFixed(1)} pts`}</td></tr>)}</tbody></table></div></section>}
