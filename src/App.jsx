...
function loadLocalState(useFixtures) {
  try {
    ...
    /* ignore invalid local rehearsal data */
  }
  return {
    users: useFixtures ? users : [],
    picksByUser: useFixtures ? clone(seededPicks) : {},
  };
}
...
const ModelPick = ({ pick, rank }) =>
  pick ? (
    <span className="model-pick" title={`${pick.team}, ${(pick.probability * 100).toFixed(1)}% win probability, rank ${rank ?? "—"}`}>
      <TeamLogo team={pick.team} />
      <strong className="model-pick-team">{pick.team}</strong>
      <span className="model-pick-probability">{(pick.probability * 100).toFixed(1)}%</span>
      {rank != null && <span className="model-pick-rank">#{rank}</span>}
    </span>
  ) : (
    <span className="missing">No pick</span>
  );
...
function StandingsTable({ board, history, pool, provisional, onProvisional, includeModels, onIncludeModels }) {
  ...
  const sortedRows = [...rows].sort((a, b) => {
    ...
    if (aValue != null && bValue != null) {
      ...
      if (comparison) return comparison * (sort.direction === "ascending" ? 1 : -1);
    }
  ...
  return (
    <section className="standings-card" aria-labelledby="standings-title">
      <div className="standings-heading">
        <div>
          <h3 id="standings-title">Standings</h3>
        </div>
        <div className="standings-options">
          <label className="provisional-toggle">
            <input type="checkbox" checked={provisional} onChange={(event) => onProvisional(event.target.checked)} /> Include provisional live scores
          </label>
          <label className="provisional-toggle">
            <input type="checkbox" checked={includeModels} onChange={(event) => onIncludeModels(event.target.checked)} /> Include model picks
          </label>
        </div>
      </div>
      <div className="table-scroll">
        <table className="standings-table" data-testid="standings-table" aria-label="Current standings">
          <thead>
            <tr>
              {columns.map(([key, label]) => (
                <th key={key} aria-sort={sortOrder(key)}>
                  <button className="table-sort-button" type="button" data-testid={`standings-sort-${key}`} onClick={() => sortBy(key)}>
                    {label}
                    {sortIndicator(key)}
                  </button>
                </th>
...
function AutopickActions({ onAutopick, probabilityKind, onProbabilityKind }) {
  return (
    <div className="pick-tools">
      <div className="pick-actions" aria-label="Autopick actions">
        <button type="button" data-testid="autopick-fpi" onClick={() => onAutopick("predictor", "FPI")}>
          Autopick by FPI
        </button>
        <button type="button" data-testid="autopick-moneyline" onClick={() => onAutopick("moneyline", "Moneyline")}>
          Autopick by Moneyline
        </button>
        <button type="button" data-testid="autopick-avg" onClick={() => onAutopick("aggregate", "AVG")}>
          Autopick by AVG
        </button>
      </div>
      <label className="pick-probability-selector">
        Probabilities{" "}
        <select aria-label="Pick probability model" value={probabilityKind} onChange={(event) => onProbabilityKind(event.target.value)}>
          <option value="predictor">FPI</option>
          <option value="moneyline">ML</option>
          <option value="aggregate">AVG</option>
        </select>
      </label>
    </div>
  );
...
function AuthPanel({ onSession }) {
  ...
  useEffect(() => {
    loadRegistrationStatus()
      .then((result) => setRegistrationOpenState(result.registrationOpen !== false))
      .catch(() => {});
  }, []);
  const submit = async (event) => {
    ...
    try {
      onSession(await authenticate(registering ? "register" : "login", values));
    } catch (failure) {
      setError(failure.code?.replaceAll("_", " ") ?? "Unable to sign in");
    } finally {
      setBusy(false);
    }
  };
    ...
    <main className="auth-shell">
      <section className="auth-panel">
        <a className="brand" href="#top">
          <span>NFL</span> Pick'em '26
        </a>
        <p className="eyebrow">2026 season</p>
        <h1>{registering ? "Create your player" : "Welcome back"}</h1>
        <p>{registering ? "Use a username and password. Email is optional and never required." : "Sign in with your username and password."}</p>
        ...
        <form onSubmit={submit}>
          {registering && (
            <label>
              Username
              <input name="username" required minLength="3" maxLength="32" pattern="[A-Za-z0-9][A-Za-z0-9_.-]{2,31}" autoComplete="username" />
            </label>
          )}
          {registering && (
            <label>
          ...
          <label>
            Password
            <input name="password" type="password" required minLength="8" autoComplete={registering ? "new-password" : "current-password"} />
          </label>
          {error && (
          ...
          <button type="submit" disabled={busy || (registering && !registrationOpen)}>
            {busy ? "Please wait..." : registering ? "Register and play" : "Sign in"}
          </button>
        </form>
        <button
...
export default function App() {
  ...
  const games = baseGames.map((game, index) =>
    activeScenario === "scheduled"
      ? {
          ...game,
          status: "scheduled",
          kickoff: new Date(Date.now() + (index + 1) * 3600000).toISOString(),
          awayScore: 0,
          homeScore: 0,
        }
      : activeScenario === "live"
        ? {
            ...
            kickoff: new Date(Date.now() + (index ? index : -1) * 3600000).toISOString(),
            homeWinProbability: index === 0 ? (game.homeWinProbability ?? 0.68) : game.homeWinProbability,
          }
        : activeScenario === "final"
          ? {
              ...game,
              status: "final",
              kickoff: new Date(Date.now() - (index + 1) * 3600000).toISOString(),
  ...
  const pickProbabilities = new Map(modelPicks(games, pickProbabilityKind).map((pick) => [pick.gameId, pick]));
  const [modelSort, setModelSort] = useState({
    key: "kickoff",
    direction: "ascending",
  });
  const sortModelsBy = (key) =>
  ...
  const sortedModelRows = [...modelRows].sort((a, b) => {
    const value = (row) =>
      ({
        ...
        probability: row.avg?.probability,
  ...
  useEffect(() => {
    ...
    const refresh = setTimeout(
      () =>
        authenticate("refresh", { refreshToken: session.refresh_token })
          .then(setSession)
          .catch(() => {
            clearSession();
            setSession(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (initial)
        setDataState((state) => ({
          ...state,
          loading: !loadedGamesByPool[poolKey],
          error: null,
        }));
      try {
        if (initial) {
          ...
          setPicks((all) => ({
            ...all,
            ...Object.fromEntries(
              result.users.map((player) => [
                player.id,
                {
                  ...all[player.id],
                  [poolKey]: result.picksByUser[player.id] ?? [],
                },
              ]),
            ),
            [session.user.id]: {
              ...all[session.user.id],
              [poolKey]: result.ownPicks,
            },
          }));
          setDataState({
            loading: false,
            error: result.games.length ? null : "No games are currently available for this pool.",
            asOf: result.asOf,
            source: result.espnSource ?? "Supabase schedule",
            cached: Boolean(result.espnCached),
          });
        } else {
          const result = await refreshLivePool(poolKey, loadedGamesByPool[poolKey] ?? [], {
            currentWeek,
            forceRefresh: refreshRequest.force,
            signal: controller.signal,
          });
        ...
        if (!controller.signal.aborted)
          setDataState((state) => ({
            ...state,
            loading: false,
            error: initial ? "Shared pick data is temporarily unavailable." : "Live ESPN data is temporarily unavailable; showing the last update.",
          }));
      } finally {
        if (!controller.signal.aborted) {
  ...
  useEffect(() => {
    ...
    const refresh = (force = false) => {
      if (document.visibilityState === "visible" && !refreshActive.current)
  ...
  }, [appUsers, picksByUser, useFixtures]);

  const registerPlayers = (event) => {
    event.preventDefault();
    const names = playerNames.map((name) => name.trim());
    if (names.some((name) => !name) || new Set(names.map((name) => name.toLocaleLowerCase())).size !== 3) {
      setMessage("ENTER 3 UNIQUE PLAYER NAMES");
      return;
    }
  ...
  const updatePick = (gameId, changes) => {
    ...
    setPicks((all) => ({
      ...all,
      [userId]: { ...all[userId], [poolKey]: nextDraft },
    }));
    setMessage("All changes saved");
  ...
  const applyModelPicks = (kind, label) => {
    ...
    setPickProbabilityKind(kind);
    setPicks((all) => ({
      ...all,
      [userId]: { ...all[userId], [poolKey]: nextDraft },
    }));
    setMessage(`${label} picks applied`);
    persistDraft(nextDraft);
  };
  ...
  useEffect(() => {
    ...
    const clearHighlight = () => setHighlightedGameId(null);
  ...
  useEffect(() => {
    ...
    const startPointerDrag = (event) => {
      ...
      pointerDrag.current = {
        ...
        sourceGameId: row.dataset.testid.replace("game-row-", ""),
      };
      try {
        row.setPointerCapture?.(event.pointerId);
      } catch {
        /* synthetic and cancelled pointers have no capture target */
      }
  ...
  const submitPasswordChange = async (event) => {
    ...
    try {
      ...
      setPasswordMessage((error.code ?? "PASSWORD_UPDATE_FAILED").replaceAll("_", " "));
    } finally {
  ...
  const submitProfileChange = async (event) => {
    ...
    if (!name || name.length > 40) {
      setProfileMessage("ENTER A DISPLAY NAME UP TO 40 CHARACTERS");
      return;
    }
    setProfileBusy(true);
    setProfileMessage("");
    try {
      ...
      setProfileMessage((error.code ?? "DISPLAY_NAME_UPDATE_FAILED").replaceAll("_", " "));
    } finally {
  ...
  return (
    <>
      <header>
        ...
        {useFixtures ? (
          <>
            <button className="setup-button" type="button" aria-label="Set up 3 players" onClick={() => setShowSetup((open) => !open)}>
              Players
            </button>
            <label className="user-switch">
              <span>Playing as</span>
              <select aria-label="Active user" value={userId} onChange={(event) => setUserId(event.target.value)}>
                {appUsers.map((user) => (
          ...
          <div className="signed-in">
            ...
            <div className={`account-actions ${showAccountMenu ? "open" : ""}`}>
              {isAdmin && (
                <button className="backup-button" type="button" data-testid="download-backup" onClick={exportBackup}>
                  Backup
                </button>
              )}
              <button
      ...
      <main id="top">
        {tab === "settings" ? (
          <section className="settings-view" aria-labelledby="settings-title">
            <div className="section-title">
              <div>
                ...
                <p>Update the name shown across the pick'em.</p>
              </div>
              <button type="button" onClick={() => setTab("overview")}>
                Back to overview
              </button>
            </div>
            <form className="profile-form" onSubmit={submitProfileChange}>
              <h2>Display name</h2>
              <label>
                Display name
                <input aria-label="Display name" value={profileDraft} onChange={(event) => setProfileDraft(event.target.value)} maxLength="40" required autoComplete="name" />
              </label>
              {profileMessage && (
                <p className={`notice ${profileMessage === "DISPLAY NAME SAVED" ? "" : "error"}`} role="status">
                  {profileMessage}
                </p>
              )}
          ...
          <>
            {useFixtures && showSetup && (
              <form className="player-setup" aria-label="Set up 3 players" onSubmit={registerPlayers}>
                <div>
                  <h2>Register 3 test players</h2>
                  <p>This replaces local test players and picks on this device.</p>
                </div>
                {playerNames.map((name, index) => (
                  <label key={index}>
                    Player {index + 1}
                    <input aria-label={`Player ${index + 1} name`} value={name} onChange={(event) => setPlayerNames((all) => all.map((item, i) => (i === index ? event.target.value : item)))} autoComplete="off" />
                  </label>
                ))}
            ...
            {!divisionWinnersOpen && <section className="hero">
              <div className="hero-actions">
                <label>
                  ...
                  <select aria-label="Week" value={poolKey} onChange={(event) => setPoolKey(event.target.value)}>
                    {enabledPools.map((item) => (
                ...
                {!useFixtures && (
                  <button type="button" title={currentWeek ? "Refresh the current week from ESPN" : "Refresh this week from ESPN, including model data"} onClick={requestRefresh} disabled={isRefreshing}>
            ...
            {!useFixtures && showPasswordForm && (
              <form className="password-form" onSubmit={submitPasswordChange}>
                ...
                {passwordMessage && (
                  <p className={`notice ${passwordMessage === "PASSWORD CHANGED" ? "" : "error"}`} role="status">
                    {passwordMessage}
                  </p>
                )}
              ...
              <section>
                <div className="section-title">
                  <div>
                    ...
                    <p>Confidence is preset from AVG strength. Drag a row to reorder it; on touch, use its handle.</p>
                  </div>
                  <div className="pick-status">
                    ...
                    {(() => {
                      const pickedCount = userPicks.filter((pick) => pick.team).length;
                      const complete = games.length > 0 && pickedCount === games.length;
                      return (
                        <span className={`pick-completion ${complete ? "complete" : "incomplete"}`} role="status">
                  ...
                  {displayedGames.map((game) => {
                    ...
                    return (
                      <article
                        ...
                        onDragOver={(event) => {
                          if (!locked && draggedGameId && draggedGameId !== game.id) {
                            event.preventDefault();
                            event.dataTransfer.dropEffect = "move";
                            if (dragOverGameId !== game.id) setDragOverGameId(game.id);
                          }
                        }}
                        onDrop={(event) => {
                          event.preventDefault();
                      ...
                      >
                        <div className="game-meta">
                          ...
                          {game.gotw && <strong className="gotw">GOTW +5</strong>}
                        </div>
                        <fieldset disabled={locked}>
                          ...
                          <div className="teams">
                            <label className={pick?.team === game.away ? "selected" : ""}>
                              <input type="radio" name={`${userId}-${game.id}`} checked={pick?.team === game.away} onChange={() => updatePick(game.id, { team: game.away })} />
                              <TeamLogo team={game.away} />
                              <span>{game.away}</span>
                              <b title={liveAway ? "ESPN real-time win probability" : `${pickProbabilityKind === "predictor" ? "FPI" : pickProbabilityKind === "moneyline" ? "ML" : "AVG"} pregame win probability`}>{liveAway ?? (game.status === "scheduled" ? (Number.isFinite(pregameHome) ? `${((1 - pregameHome) * 100).toFixed(0)}%` : "—") : game.awayScore)}</b>
                            </label>
                            <span className="at">@</span>
                            <label className={pick?.team === game.home ? "selected" : ""}>
                              <input type="radio" name={`${userId}-${game.id}`} checked={pick?.team === game.home} onChange={() => updatePick(game.id, { team: game.home })} />
                              <TeamLogo team={game.home} />
                              <span>{game.home}</span>
                              <b title={liveHome ? "ESPN real-time win probability" : `${pickProbabilityKind === "predictor" ? "FPI" : pickProbabilityKind === "moneyline" ? "ML" : "AVG"} pregame win probability`}>{liveHome ?? (game.status === "scheduled" ? (Number.isFinite(pregameHome) ? `${(pregameHome * 100).toFixed(0)}%` : "—") : game.homeScore)}</b>
                            </label>
                          </div>
                        </fieldset>
                        <div className="confidence-cell">
                          <span className="drag-handle" aria-hidden="true" data-testid={`confidence-drag-${game.id}`} data-game-id={game.id}>
                            ⠿
                          </span>
                          <label className="confidence">
                            <span>Rank</span>
                            <select
                              aria-label={`${game.away} at ${game.home} confidence`}
            ...
            {tab === "models" && (
              <section>
                <div className="section-title">
                  <div>
                    <h2>Model picks</h2>
                    <p>ESPN FPI probability and normalized no-vig moneyline, equal weighted.</p>
                  </div>
                </div>
                <div className="table-scroll">
                  <table className="models-table">
                    <thead>
                      <tr>
                        <th aria-sort={modelSortOrder("kickoff")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-game" onClick={() => sortModelsBy("kickoff")}>
                            Game{modelSortIndicator("kickoff")}
                          </button>
                        </th>
                        <th className="model-pick-column" aria-sort={modelSortOrder("fpi")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-fpi" onClick={() => sortModelsBy("fpi")}>
                            FPI{modelSortIndicator("fpi")}
                          </button>
                        </th>
                        <th className="model-rank-column" aria-sort={modelSortOrder("fpiRank")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-fpi-rank" onClick={() => sortModelsBy("fpiRank")}>
                            Rank{modelSortIndicator("fpiRank")}
                          </button>
                        </th>
                        <th className="model-pick-column" aria-sort={modelSortOrder("moneyline")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-moneyline" onClick={() => sortModelsBy("moneyline")}>
                            Moneyline{modelSortIndicator("moneyline")}
                          </button>
                        </th>
                        <th className="model-rank-column" aria-sort={modelSortOrder("moneylineRank")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-moneyline-rank" onClick={() => sortModelsBy("moneylineRank")}>
                            Rank{modelSortIndicator("moneylineRank")}
                          </button>
                        </th>
                        <th className="model-pick-column" aria-sort={modelSortOrder("avg")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-avg" onClick={() => sortModelsBy("avg")}>
                            AVG{modelSortIndicator("avg")}
                          </button>
                        </th>
                        <th className="model-rank-column" aria-sort={modelSortOrder("avgRank")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-avg-rank" onClick={() => sortModelsBy("avgRank")}>
                            Rank{modelSortIndicator("avgRank")}
                          </button>
                        </th>
                        <th className="model-probability-column" aria-sort={modelSortOrder("probability")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-probability" onClick={() => sortModelsBy("probability")}>
                            Pick probabilities FPI / ML / AVG
                            {modelSortIndicator("probability")}
                          </button>
                        </th>
                        <th className="model-disagreement-column" aria-sort={modelSortOrder("disagreement")}>
                          <button className="table-sort-button" type="button" data-testid="models-sort-disagreement" onClick={() => sortModelsBy("disagreement")}>
                            Disagreement{modelSortIndicator("disagreement")}
                          </button>
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {sortedModelRows.map(({ game, fpi, moneyline: ml, avg: pick, disagreement }) => (
                        <tr key={game.id}>
                          <td>
                            <div className="model-game">
                              <span className="model-matchup">
                                <TeamLogo team={game.away} />
                                {game.away}
                                <span>@</span>
                                <TeamLogo team={game.home} />
                              ...
                              <time dateTime={game.kickoff}>
                                {formatCETWeekday(game.kickoff)} {formatCETTime(game.kickoff)}
                              </time>
                            </div>
                          </td>
                          <td className="model-pick-column">
                            <ModelPick pick={fpi} rank={fpi?.confidence} />
                          </td>
                          <td className="model-rank-column">{fpi?.confidence ?? "—"}</td>
                          <td className="model-pick-column">
                            <ModelPick pick={ml} rank={ml?.confidence} />
                          </td>
                          <td className="model-rank-column">{ml?.confidence ?? "—"}</td>
                          <td className="model-pick-column">
                            <ModelPick pick={pick} rank={pick?.confidence} />
                          </td>
                          <td className="model-rank-column">{pick?.confidence ?? "—"}</td>
                          <td className="model-probability-column">{[fpi, ml, pick].map((item) => (item ? `${(item.probability * 100).toFixed(1)}%` : "—")).join(" / ")}</td>
                          <td className="model-disagreement-column">{disagreement === null ? "Missing inputs" : `${(disagreement * 100).toFixed(1)}`}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {tab === "admin" && isAdmin && (
              <AdminPanel
                ...
                onPicksUpdated={(playerId, picks) =>
                  setPicks((all) => ({
                    ...all,
                    [playerId]: { ...all[playerId], [poolKey]: picks },
                  }))
                }
                onPlayerDeleted={(playerId) => {
                  setAppUsers((all) => all.filter((player) => player.id !== playerId));
                  setPicks((all) => {
                    const next = { ...all };
                ...
                onGamesUpdated={(updatedPoolKey, updatedGames) => {
                  setLoadedGamesByPool((all) => ({
                    ...all,
                    [updatedPoolKey]: updatedGames,
                  }));
                  setChartData((all) =>
                    all
                      ? {
            ...
            {tab === "rules" && (
              <section className="rules">
                <h2>Rules</h2>
                <h3>Confidence</h3>
                <p>Use each value from 1 through the number of games exactly once in a completed pool. Locked games keep their values.</p>
                <h3>Scoring</h3>
                <p>A correct pick earns its confidence. Game of the Week adds five. A final tie awards every submitted team pick full points.</p>
                <h3>Live results</h3>
      ...
      <footer>
        {divisionWinnersOpen && divisionStandingsStatus ? (
          ...
          <span data-testid="real-data-status">
            {dataState.source === "ESPN" ? "Browser ESPN data" : (dataState.source ?? "Supabase schedule")}
            {dataState.cached ? " · cached" : ""} · {games.length} games · {dateRange(games)} · pregame probabilities for {games.filter((game) => Number.isFinite(game.predictorHome) || (Number.isFinite(game.homeMoneyline) && Number.isFinite(game.awayMoneyline))).length}/{games.length} · <time dateTime={dataState.asOf ?? new Date().toISOString()}>{dataState.asOf ? `Updated ${new Intl.DateTimeFormat("en", { timeZone: "Europe/Berlin", hour: "numeric", minute: "2-digit" }).format(new Date(dataState.asOf))}` : "Loading"}</time>
          </span>
        ) : (
          "Loading"
        )}{" "}
