...
const ModelPick = ({ pick, rank }) => pick ? <span className="model-pick" title={`${pick.team}, rank ${rank ?? '—'}`}><TeamLogo team={pick.team} /><strong className="model-pick-team">{pick.team}</strong>{rank != null && <span className="model-pick-rank">#{rank}</span>}</span> : <span className="missing">No pick</span>
