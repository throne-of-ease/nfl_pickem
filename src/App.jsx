...
function StandingsTable({ board, history, pool, provisional, onProvisional, includeModels, onIncludeModels }) {
  ...
  return <section className="standings-card" aria-labelledby="standings-title">
    ...
    <div className="table-scroll"><table className="standings-table" data-testid="standings-table" aria-label="Current standings"><thead><tr><th>Rank</th><th>Player</th><th>Points</th><th>Total points</th><th>GOTW points</th><th>GOTW % of total</th><th>Without GOTW</th><th>Pick %</th><th>Point %</th><th>Games picked</th></tr></thead><tbody>{board.map((player, index) => { const season = seasonByPlayer.get(player.id); const totalPoints = season?.cumulative?.at(-1) ?? player.points; const gotwPoints = season?.gotw ?? 0; return <tr key={player.id}><th scope="row">{index + 1}</th><td><strong>{player.name}</strong></td><td>{player.points}</td><td data-testid={`total-points-${player.id}`}>{totalPoints}</td><td data-testid={`gotw-points-${player.id}`}>{gotwPoints}</td><td data-testid={`gotw-share-${player.id}`}>{totalPoints ? `${(gotwPoints / totalPoints * 100).toFixed(1)}%` : '—'}</td><td data-testid={`without-gotw-${player.id}`}>{totalPoints - gotwPoints}</td><td>{player.played ? `${(player.correct / player.played * 100).toFixed(1)}%` : '—'}</td><td>{player.maximum ? `${(player.points / player.maximum * 100).toFixed(1)}%` : '—'}</td><td>{player.picksMade}</td></tr> })}</tbody></table></div>
  </section>
}
