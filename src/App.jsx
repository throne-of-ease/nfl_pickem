...
import { authenticate, clearSession, isCurrentPool, loadChartData, loadPool, loadRegistrationStatus, refreshLivePool, restoreSession, savePicks } from './api.js'
...
export default function App() {
  ...
  return <>
    ...
    <main id="top">
      ...
      <section className="hero"><div><p className="eyebrow">2026 season</p><h1>{pool.label}</h1>{pool.phase === 'preseason' && <p className="rehearsal">Rehearsal — does not count.</p>}</div><div className="hero-actions"><label>Pool <select aria-label="Pool" value={poolKey} onChange={(event) => setPoolKey(event.target.value)}>{enabledPools.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></label>{!useFixtures && <button type="button" title={currentWeek ? 'Refresh the current week from ESPN' : 'Older weeks use their cached slate'} onClick={() => setRefreshVersion((version) => version + 1)} disabled={isRefreshing || Boolean(baseGames.length) && !currentWeek}>{isRefreshing ? 'Refreshing…' : currentWeek ? 'Refresh' : 'Historical cache'}</button>}</div></section>
