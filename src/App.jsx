...
import { authenticate, clearSession, loadChartData, loadPool, loadRegistrationStatus, restoreSession, savePicks } from './api.js'
...
export default function App() {
  ...
  const games = baseGames.map((game, index) => activeScenario === 'scheduled' ? { ...game, status: 'scheduled', kickoff: new Date(Date.now() + (index + 1) * 3600000).toISOString(), awayScore: 0, homeScore: 0 } : activeScenario === 'live' ? { ...game, status: index === 0 ? 'live' : 'scheduled', kickoff: new Date(Date.now() + (index ? index : -1) * 3600000).toISOString() } : activeScenario === 'final' ? { ...game, status: 'final', kickoff: new Date(Date.now() - (index + 1) * 3600000).toISOString() } : activeScenario === 'missing-data' ? { ...game, predictorHome: null, homeMoneyline: null, awayMoneyline: null } : game)
  ...
  const persistDraft = (draft) => {
    recordPickBackup(backupAccountKey, poolKey, draft)
    if (useFixtures || !session?.access_token) return
  ...
  return <>
    <header><a className="brand" href="#top"><span>NFL</span> PICK/26</a><nav aria-label="Main navigation">{[['overview', 'Overview'], ['games', 'My picks'], ['standings', 'Standings'], ['charts', 'Charts'], ['models', 'Models'], ['rules', 'Rules'], ...(isAdmin ? [['admin', 'Admin']] : [])].map(([item, label]) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{label}</button>)}</nav>{useFixtures ? <><button className="backup-button" type="button" data-testid="download-backup" onClick={exportBackup}>Backup</button><button className="setup-button" type="button" aria-label="Set up 3 players" onClick={() => setShowSetup((open) => !open)}>Players</button><label className="user-switch"><span>Playing as</span><select aria-label="Active user" value={userId} onChange={(event) => setUserId(event.target.value)}>{appUsers.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label></> : <div className="signed-in"><span>{viewerName || session.user.user_metadata?.display_name || session.user.user_metadata?.username || 'Player'}</span><button className="backup-button" type="button" data-testid="download-backup" onClick={exportBackup}>Backup</button><button type="button" onClick={() => { clearSession(); setSession(null); setAppUsers([]); setPicks({}); setIsAdmin(false) }}>Sign out</button></div>}</header>
    <main id="top">
