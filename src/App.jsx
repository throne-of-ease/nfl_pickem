...
import { POOLS, buildSeasonHistory, isLocked, modelAutopick, modelDisagreement, modelPicks, noVigProbabilities, poolMetrics, presetConfidencePicks, standings, validateDraft } from './domain.js'
...
function StandingsTable({ board, history, pool, provisional, onProvisional, includeModels, onIncludeModels }) {
  ...
  const rows = board.map((player, index) => {
    ...
    return {
      ...
      totalPoints,
      gotwPoints,
      gotwShare: totalPoints ? gotwPoints / totalPoints * 100 : null,
  ...
  const columns = [
    ['rank', 'Rank'], ['name', 'Player'], ['points', 'Points'], ['totalPoints', 'Total points'], ['gotwPoints', 'GOTW points'],
    ['gotwShare', 'GOTW % of total'], ['withoutGotw', 'Without GOTW'], ['pickPercentage', 'Pick %'], ['pointPercentage', 'Point %'], ['gamesPicked', 'Games picked'],
  ]
  ...
  return <section className="standings-card" aria-labelledby="standings-title">
    ...
    <div className="table-scroll"><table className="standings-table" data-testid="standings-table" aria-label="Current standings"><thead><tr>{columns.map(([key, label]) => <th key={key} aria-sort={sortOrder(key)}><button className="table-sort-button" type="button" data-testid={`standings-sort-${key}`} onClick={() => sortBy(key)}>{label}{sortIndicator(key)}</button></th>)}</tr></thead><tbody>{sortedRows.map(({ player, rank, totalPoints, gotwPoints, gotwShare, withoutGotw, pickPercentage, pointPercentage, gamesPicked }) => <tr key={player.id}><th scope="row">{rank}</th><td><strong>{player.name}</strong></td><td>{player.points}</td><td data-testid={`total-points-${player.id}`}>{totalPoints}</td><td data-testid={`gotw-points-${player.id}`}>{gotwPoints}</td><td data-testid={`gotw-share-${player.id}`}>{gotwShare == null ? '—' : `${gotwShare.toFixed(1)}%`}</td><td data-testid={`without-gotw-${player.id}`}>{withoutGotw}</td><td>{pickPercentage == null ? '—' : `${pickPercentage.toFixed(1)}%`}</td><td>{pointPercentage == null ? '—' : `${pointPercentage.toFixed(1)}%`}</td><td>{gamesPicked}</td></tr>)}</tbody></table></div>
  </section>
}

function AutopickActions({ onAutopick }) {
  return <div className="pick-actions" aria-label="Autopick actions"><button type="button" data-testid="autopick-fpi" onClick={() => onAutopick('predictor', 'FPI')}>Autopick by FPI</button><button type="button" data-testid="autopick-moneyline" onClick={() => onAutopick('moneyline', 'Moneyline')}>Autopick by Moneyline</button><button type="button" data-testid="autopick-avg" onClick={() => onAutopick('aggregate', 'AVG')}>Autopick by AVG</button></div>
}
...
export default function App() {
  ...
  const updatePick = (gameId, changes) => {
    ...
    if (occupied && isLocked(games.find((game) => game.id === occupied.gameId), new Date(), pool.acceptsLatePicks)) { setMessage('LOCKED VALUE CANNOT BE REUSED'); return false }
    ...
    if (!result.ok) { setMessage(result.code.replaceAll('_', ' ')); return false }
  ...
  const applyModelPicks = (kind, label) => {
    const nextDraft = modelAutopick(games, kind, userPicks, { acceptsLatePicks: pool.acceptsLatePicks })
    const result = validateDraft(games, nextDraft, { previous: userPicks, acceptsLatePicks: pool.acceptsLatePicks })
