...
export default function App() {
  ...
  return <>
    ...
    <main id="top">
      ...
      {!useFixtures && games.length > 0 && <p className="notice" role="status" data-testid="real-data-status">{dataState.source === 'ESPN' ? 'Browser ESPN' : dataState.source ?? 'Supabase schedule'}{dataState.cached ? ' · cached' : ''} · {games.length} games · {dateRange(games)} · pregame probabilities for {games.filter((game) => Number.isFinite(game.predictorHome) || Number.isFinite(game.homeMoneyline) && Number.isFinite(game.awayMoneyline)).length}/{games.length}{dataState.asOf ? ` · updated ${new Intl.DateTimeFormat('en', { timeZone: 'Europe/Berlin', hour: 'numeric', minute: '2-digit' }).format(new Date(dataState.asOf))}` : ''}</p>}
