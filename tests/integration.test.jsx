...
describe('four-user application flow', () => {
  ...
  it('shows tracker metrics by default, includes live clock detail, and sorts by GQ and Dev', async () => {
    ...
    const liveGame = liveRow.querySelector('.overview-game')
    expect(liveGame.children[0]).toHaveClass('overview-game-line')
    expect(liveGame.children[1]).toHaveClass('live-badge')
  ...
  it('renders all five accessible charts and every display mode', async () => {
    ...
    expect(screen.getAllByRole('button', { name: 'Download chart as PNG' })).toHaveLength(5)
    expect(screen.getAllByRole('button', { name: 'Share chart as PNG' })).toHaveLength(5)
    const aggressivenessModel = screen.getByLabelText('Aggressiveness index model')
    const aggressivenessView = screen.getByLabelText('Aggressiveness index view')
    expect(within(aggressivenessModel).getAllByRole('option')).toHaveLength(3)
    expect(within(aggressivenessView).getAllByRole('option')).toHaveLength(3)
    expect(aggressivenessModel).toHaveValue('aggregate')
    expect(aggressivenessView).toHaveValue('weekly')
    expect(screen.getByRole('img', { name: 'Aggressiveness index by week, aggregate' })).toBeInTheDocument()
    await user.selectOptions(aggressivenessModel, 'predictor')
    expect(screen.getByRole('img', { name: 'Aggressiveness index by week, predictor' })).toBeInTheDocument()
    await user.selectOptions(aggressivenessView, 'selected_week')
    expect(screen.getByRole('img', { name: 'Aggressiveness index for selected week, predictor' })).toBeInTheDocument()
    await user.selectOptions(aggressivenessView, 'season_average')
    expect(screen.getByRole('img', { name: 'Aggressiveness index season average, predictor' })).toBeInTheDocument()
