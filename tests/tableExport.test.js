...
it('exports full names and displayed picks without exposing hidden picks', async () => {
  document.body.innerHTML = `<table><thead><tr><th><div class="overview-player"><strong>Very long player name</strong><b>12</b><span>LEAD</span><small>5/-2/9</small></div></th><th>Other player</th></tr></thead><tbody><tr><td><span class="pick-hidden">?</span></td><td><div class="overview-pick"><strong class="pick-team" style="display:none">PHI</strong><span>7</span></div></td></tr></tbody></table>`
  ...
  const context = { measureText: text => ({ width: text.length * 8 }), scale: vi.fn(), fillRect: vi.fn(), strokeRect: vi.fn(), fillText: vi.fn() }
  ...
  expect(lines).toContain('PHI')
  expect(lines).toContain('7')
})

it('reports an unavailable canvas', () => {
  vi.spyOn(HTMLCanvasElement.prototype, 'getContext').mockReturnValue(null)
  expect(() => tableToPngBlob(null, 'Week 1')).toThrow('Canvas is unavailable')
})
