...
test('four users, charts, pools, and responsive states work', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('svg.chart')).toHaveCount(5)
  await expect(page.getByRole('button', { name: 'Download chart as PNG' })).toHaveCount(5)
  await expect(page.getByRole('button', { name: 'Share chart as PNG' })).toHaveCount(5)
  await expect(page.getByLabel('Aggressiveness index model')).toHaveValue('aggregate')
  await expect(page.getByLabel('Aggressiveness index view')).toHaveValue('weekly')
  await expect(page.getByRole('img', { name: 'Aggressiveness index by week, aggregate' })).toBeVisible()
  await page.getByLabel('Aggressiveness index model').selectOption('predictor')
  await page.getByLabel('Aggressiveness index view').selectOption('selected_week')
  await expect(page.getByRole('img', { name: 'Aggressiveness index for selected week, predictor' })).toBeVisible()
  await page.getByLabel('Aggressiveness index view').selectOption('season_average')
  await expect(page.getByRole('img', { name: 'Aggressiveness index season average, predictor' })).toBeVisible()
...
test('production registration starts a session without email confirmation', async ({ page }, testInfo) => {
  ...
  await expect(page.getByLabel('Week', { exact: true })).toHaveValue('week-02')
  await expect(page.getByText('2026 season', { exact: true })).toHaveCount(0)
  await expect(page.getByText(/Rehearsal.*does not count/)).toHaveCount(0)
